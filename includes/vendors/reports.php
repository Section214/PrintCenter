 ...
 * Vendor reports
 *
 * @package     PrintCenter\Vendor\Reports
 * @since       1.0.0
 */
 ...
 * @return      void
 */
function printcenter_product_vendors_report_overview() {
	...
	<div id="poststuff" class="woocommerce-reports-wrap">
		<div class="woocommerce-reports-sidebar">
			<div class="postbox">
				<h3><span><?php _e( 'Total sales', 'printcenter' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if( $total_sales > 0 ) { echo woocommerce_price( $total_sales ); } else { _e( 'n/a', 'printcenter' ); } ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Total orders', 'printcenter' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if( $total_orders > 0 ) { echo $total_orders; } else { _e( 'n/a', 'printcenter' ); } ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Average order total', 'printcenter' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if( $total_orders > 0 ) { echo woocommerce_price( $total_sales / $total_orders ); } else { _e( 'n/a', 'woocommerce' ); } ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Total earned', 'printcenter' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if( $total_earnings > 0 ) { echo woocommerce_price( $total_earnings ); } else { _e( 'n/a', 'woocommerce' ); } ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Total earned by vendors', 'printcenter' ); ?></span></h3>
				<div class="inside">
		...
		<div class="woocommerce-reports-main">
			<div class="postbox">
				<h3><span><?php _e( 'This month\'s sales', 'printcenter' ); ?></span></h3>
	...
	for( $date = $start_date; $date <= $end_date; $date = strtotime( '+1 day', $date ) ) {
		...
		$chart_data[ __( 'Total earned by vendors', 'printcenter' ) ][] = array(
			$date . '000',
			$day_total_vendors
		);

		$chart_data[ __( 'Number of orders', 'printcenter' ) ][] = array(
			$date . '000',
			$order_count
		);
	}
	...
	<script type="text/javascript">
		jQuery(function(){
			...
			var plot = jQuery.plot(placeholder, [
				...
				foreach ( $chart_data as $name => $data ) {
					if( $name == __( 'Number of orders', 'printcenter' ) ) {
 ...
 * @return      void
 */
function printcenter_product_vendors_report_vendor_sales() {
	...
	<form method="post" action="">
		<p><select id="vendor" name="vendor" class="ajax_chosen_select_vendor" data-placeholder="<?php _e( 'Search for a vendor&hellip;', 'woo_vendors' ); ?>" style="width: 400px;"><?php echo $option; ?></select> <input type="submit" style="vertical-align: top;" class="button" value="<?php _e( 'Show', 'woocommerce' ); ?>" /></p>
		<script type="text/javascript">
			jQuery(function(){
	...
	if( $chosen_product_ids && is_array( $chosen_product_ids ) ) {
		...
		?>
		<h4><?php printf( __( 'Sales and earnings for %s:', 'printcenter' ), $vendor->title ); ?></h4>
		<table class="bar_chart">
			<thead>
				<tr>
					<th><?php _e( 'Month', 'printcenter' ); ?></th>
					<th colspan="2"><?php _e( 'Sales', 'printcenter' ); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php
